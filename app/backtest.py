...
def backtest(ticker, ticker_df, num_shares=10, initial_capital=10000):
    ...
    # Create subplots
    fig = make_subplots(rows=3, cols=1, row_heights=[0.5, 0.2, 0.3], shared_xaxes=True, vertical_spacing=0.07)

    # Total
    fig.add_trace(go.Scatter(x=portfolio.index, y=portfolio["total"], mode="lines", opacity=0.9, name="Portfolio Total"), row=1, col=1)
    ...
    fig.add_trace(go.Scatter(x=portfolio.index, y=portfolio["holdings"], mode="lines", opacity=0.8, name="Holding Value"), row=2, col=1)
    fig.add_trace(go.Scatter(x=portfolio.index, y=portfolio["cash"], mode="lines", opacity=0.8, name="Cash"), row=2, col=1)
    
    # Close
    fig.add_trace(go.Scatter(x=ticker_df.index, y=ticker_df["Close"], mode="lines", opacity=0.8, name="Close"), row=3, col=1)
    # Add up triangles for buy signals and sell signals at the identified points
    fig.add_trace(go.Scatter(
